...
import { AccountId, BlockNumber, Exposure, Option, RewardDestination, StakingLedger, Tuple, ValidatorPrefs, Vector, UnlockChunk, SessionKeys } from '@polkadot/types';
...
function getNextSessionId (_nextKeyFor: Option<Keys | SessionKey>): AccountId | undefined {
...
function unwrapSessionIds (stashId: AccountId, validatorIds: AccountId[], auraIds: AccountId[], nextKeys: Option<Keys | SessionKey>): { nextSessionId?: AccountId; sessionId?: AccountId } {
  ...
  return {
    nextSessionId: getNextSessionId(nextKeys),
    sessionId: getNextSessionId(nextKeys)
  };
}
...
function withStashController (api: ApiInterfaceRx, accountId: AccountId, controllerId: AccountId): Observable<DerivedStaking> {
  ...
  return (
    ...
    ]) as Observable<[BN, BlockNumber, AccountId[], [Option<SessionKey>, AccountId[], Option<StakingLedger>, [AccountId[]], RewardDestination, Exposure, [ValidatorPrefs]]]>
  ).pipe(
...
function withControllerLedger (api: ApiInterfaceRx, accountId: AccountId, stakingLedger: StakingLedger): Observable<DerivedStaking> {
  ...
  return (
    ...
    ]) as Observable<[AccountId[], [Option<SessionKey>, AccountId[], [AccountId[]], RewardDestination, Exposure, [ValidatorPrefs]]]>
  ).pipe(
