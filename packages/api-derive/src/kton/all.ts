...
import { DerivedKtonBalances } from '../types';
import { drr } from '../util/drr';

type Result = [AccountId | undefined, BlockNumber | undefined, [Balance?,  BalanceLock[]?]];

const EMPTY_ACCOUNT = new AccountId();
const ZERO = new Balance(0);

function calcBalances ([accountId = EMPTY_ACCOUNT, bestNumber = ZERO, [freeBalance = ZERO, locks = []]]: Result): DerivedKtonBalances {
  ...
  const availableBalance: BN = bnMax(ZERO, freeBalance.sub(lockedBalance));
  ...
  return {
    ...
    freeBalance,
    lockedBalance,
  } as unknown as DerivedKtonBalances;
}
 ...
 * ```
 */
export function all (api: ApiInterfaceRx): (address: AccountIndex | AccountId | Address | string) => Observable<DerivedKtonBalances> {
  return (address: AccountIndex | AccountId | Address | string): Observable<DerivedKtonBalances> => {
      ...
      switchMap(([accountId]): Observable<Result> =>
        (accountId
          ? combineLatest([
            ...
            bestNumber(api)(),
            api.queryMulti([
              [api.query.kton.freeBalance, accountId],
              [api.query.kton.locks, accountId]
            ])
          ])
          : of([undefined, undefined, [undefined, undefined]])
